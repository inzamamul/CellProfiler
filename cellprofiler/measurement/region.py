...
def add_object_count_measurements(measurements, object_name, object_count):
    ...
    measurements.add_measurement('Image', FF_COUNT % object_name, numpy.array([object_count], dtype=float))


def add_object_location_measurements(measurements, object_name, labels, object_count=None):
    ...
    #
    if object_count:
        centers = scipy.ndimage.center_of_mass(numpy.ones(labels.shape), labels, range(1, object_count + 1))
    ...
    measurements.add_measurement(object_name, M_LOCATION_CENTER_X, location_center_x)

    measurements.add_measurement(object_name, M_LOCATION_CENTER_Y, location_center_y)

    measurements.add_measurement(object_name, M_NUMBER_OBJECT_NUMBER, number)


def add_object_location_measurements_ijv(measurements, object_name, ijv, object_count=None):
    ...
    measurements.add_measurement(object_name, M_NUMBER_OBJECT_NUMBER, numpy.arange(1, object_count + 1))
...
def get_object_measurement_columns(object_name):
    ...
    '''
    return [
        (object_name, M_LOCATION_CENTER_X, cellprofiler.measurement.COLTYPE_FLOAT),
